...
import {
  protectDualGPXFiles,
  DualGPXProtectionResult,
} from "@/lib/gpxProtection";
...
export default function GPXUploader({
  ...
  const validateGPXFile = (file: File): Promise<GPXFile> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        console.log(
          "File content read successfully, size:",
          content.byteLength
        );
        resolve({
          file,
  ...
  const protectDualGPXFilesHandler = async () => {
    if (!dataProtectorCore || !gpx1 || !gpx2) {
      alert(
        "Please ensure both GPX files are uploaded and wallet is connected"
      );
      return;
    }
    ...
    <div className="space-y-6 p-6 bg-white rounded-lg border border-gray-200">
      <div className="text-center">
        ...
        <p className="text-gray-600">
          Upload two GPX files to create a protected dataset
        </p>
      </div>
        ...
        <div className="flex space-x-4">
          <label className="flex items-center">
            <input
              ...
              onChange={(e) =>
                setUploadMode(e.target.value as "ipfs" | "arweave")
              }
              disabled={isProtecting}
              className="mr-2"
            />
